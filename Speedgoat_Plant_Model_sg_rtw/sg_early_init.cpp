...
void sg_init_sfcns(void)
{
    ...
    rtssSetErrorStatusPtr(&sgEarlyInitSfcnInfo, (&rtmGetErrorStatus(Speedgoat_Plant_Model_M)));
    ...
    
    {
        ...
        // call the i/o extension init functions
            // -21
                ...
                if (useAxiI2cFlag){
                    ErrCode = IO3xx_21_update2(devname, 1, 10922U, 0U, 0U, 0);
                } else {
                    IO3xx_21_update(devname, 1, 10922U, 0U, 0U,0);
                    ErrCode = 0;
                }
        ...
        sgDmaSharedState->dmaTriggerTimeTicks   = (uint32_t)(0.0001 * 75000000U);
        ...
        modelTimingsPtr->fpgaFrequency = 100000000U;
        modelTimingsPtr->sampleTime = 0.0001;
